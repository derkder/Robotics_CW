...
%num_epochs = 1;
for k = 1:num_epochs
    x_k_minus = Phi_k_minus_1 * x_k;
    P_k_minus = Phi_k_minus_1 *  P_k * Phi_k_minus_1' + Q_k_minus_1;

    % 计算每颗卫星的距离
    for idx = 1:num_satellites
        estimated_position = x_k_minus(1:3);
        pseudo_range_rates = data_rates(k + 1, 2:end); % 假设第二行是伪距率 
        predicted_receiver_velocity = x_k_minus(4:6); % 从状态向量中获取用户预测速度
        j = satellite_numbers(idx);
        [sat_r_es_e, sat_v_es_e] = Satellite_position_and_velocity(k - 1, j);
        ...
        second = predicted_receiver_velocity + Omega_ie * x_k_minus(1 : 3);
    ...
    for j = satellite_numbers
        measurement_innovation(idx) = data(k + 1, idx + 1)- satellite_ranges(idx) - x_k_minus(7);
        measurement_innovation(idx + num_satellites) = data_rates(k + 1, idx + 1)- satellite_range_rates(idx) - x_k_minus(8);
        idx = idx + 1;
    end
    
    x_k = x_k_minus + K_k * measurement_innovation;
    P_k = (eye(8) - K_k * H_k) * P_k_minus;
    [L_b, lambda_b, h_b, v_eb_n] = pv_ECEF_to_NED(x_k(1 : 3), x_k(4 : 6));
    fprintf('最终结果：time： %f° /n 纬度 = %f°, 经度 = %f°, 高度 = %f米\n, 速度 = %f米%f米%f米\n', k - 1, L_b / deg_to_rad, lambda_b / deg_to_rad, h_b, v_eb_n(1), v_eb_n(2), v_eb_n(3));
end
